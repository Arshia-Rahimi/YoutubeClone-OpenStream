package com.github.openstream.ui.global.reusable.popups.inputdialog
...
class InputDialogViewModel(
    ...
    fun confirm() {
        when (type) {
            is InputType.CreatePlaylist -> playlistRepo.createPlaylist(input.value).onEach {
                when (it) {
                    is Resource.Error -> {
            ...
            is InputType.SearchPlaylist -> playlistRepo.getPlaylist(input.value)
                .onEach { fetchedPlaylist ->
